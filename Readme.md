# EGO-Planner-Swarm（ROS 2 Lyrical 移植版）
...
上游 master 是 ROS 1 (catkin) 版本。本分支把它整体迁到 **ROS 2 Lyrical**（576 个文件变更），其中最硬的两块：

### 9.1 自研兼容包 `ament_target_dependencies_compat`

Lyrical **删除了** `ament_target_dependencies()` 宏，19 个包全部依赖它。这个包用自定义 CMake 宏复刻了原行为（优先现代 target `${dep}_TARGETS` 递归展开，回退经典变量 `${dep}_INCLUDE_DIRS`/`${dep}_LIBRARIES`），顺手修了两个上游 bug：

1. **VTK 的 `JsonCpp::JsonCpp` 悬空 target**——PCL 拉进来的 VTK 配置引用了它却从不 `find_package(jsoncpp)`，配置阶段直接报错。在兼容层里补上定义
2. **`ament_get_recursive_properties()` 指数级展开**——原实现沿 `INTERFACE_LINK_LIBRARIES` 递归**不记已访问节点**，遇到 `pcl_ros` 那样密集的导入目标图会指数爆炸，`mockamap` 配置卡死 8 分钟以上。改成**记忆化 BFS**

### 9.2 全仓库 API 适配

- 头文件改名：`.h → .hpp`（`message_filters`、`tf2/LinearMath`、`tf2/convert`、`cv_bridge`）
- `grid_map.cpp`：`message_filters::Subscriber` 第三参 `rmw_qos_profile_t` → `const rclcpp::QoS&`
- 所有节点 `ros::NodeHandle` → `rclcpp::Node::SharedPtr`，参数改 `declare_parameter`/`get_parameter`
- 时间类型 `ros::Time` → `rclcpp::Time`（集群广播里的时钟类型转换是最易出错处之一）
- 消息定义迁到 `rosidl`，删掉 ROS 1 的 Python 消息包装
- **删除** ROS 1 专属件：`rviz_plugins`（RViz 插件）、catkin 的 `cmake/`、所有 `.launch` XML
